...
export default function Register() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 font-sans relative overflow-hidden py-12">
      ...
      <div className="fixed top-[-10%] left-[-10%] w-[500px] h-[500px] bg-red-400/20 rounded-full mix-blend-multiply filter blur-3xl opacity-70 animate-blob pointer-events-none"></div>
      <div className="fixed top-[20%] right-[-10%] w-[500px] h-[500px] bg-rose-400/20 rounded-full mix-blend-multiply filter blur-3xl opacity-70 animate-blob animation-delay-2000 pointer-events-none"></div>
      <div className="fixed bottom-[-20%] left-[20%] w-[500px] h-[500px] bg-pink-400/20 rounded-full mix-blend-multiply filter blur-3xl opacity-70 animate-blob animation-delay-4000 pointer-events-none"></div>
      ...
      <div className="w-full max-w-[500px] glass-panel mac-shadow rounded-2xl p-8 sm:p-10 z-10 relative">
        ...
        <div className="flex bg-slate-100/60 backdrop-blur-md p-1 rounded-xl mb-8 border border-slate-200/50">
          <button 
            type="button"
            className={`flex-1 py-2.5 text-sm font-semibold transition-all rounded-lg ${role === 'patient' ? 'bg-white text-rose-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
          ...
          <button 
            type="button"
            className={`flex-1 py-2.5 text-sm font-semibold transition-all rounded-lg ${role === 'doctor' ? 'bg-white text-rose-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {role === 'doctor' && (
            <div className="space-y-5 pt-2 mt-2 border-t border-slate-100">
              <div className="bg-rose-50/50 border border-rose-100 rounded-xl p-4 text-sm text-rose-800">
                ...
                <p className="text-rose-700/80 text-xs">After registration, you will need to complete your profile and upload your CV for administrator approval.</p>
              </div>
              ...
              <div>
                ...
                <textarea 
                  ...
                  className="w-full px-4 py-2.5 bg-white/70 border border-slate-200 rounded-xl focus:ring-4 focus:ring-rose-500/10 focus:border-rose-400 outline-none transition-all text-sm resize-none placeholder:text-slate-400"
              ...
              <div>
                ...
                <div className="border-2 border-dashed border-slate-300 bg-white/50 rounded-xl p-6 text-center text-slate-500 text-sm hover:bg-white transition-colors relative" data-element-id="cv-upload">
                  ...
                  {cvFile ? <span className="text-rose-600 font-semibold">{cvFile.name}</span> : <span>📄 Drag & drop or click to browse</span>}
                </div>
                {isUploading && role === 'doctor' && (
                  <div className="mt-3">
                    ...
                    <div className="w-full bg-slate-200 rounded-full h-2">
                      <div className="bg-gradient-to-r from-rose-500 to-pink-500 h-2 rounded-full transition-all duration-300" style={{ width: `${uploadProgress}%` }}></div>
                    </div>
                  </div>
                )}
              </div>
        ...
        <div className="text-center mt-8 text-sm text-slate-500">
          Already have an account? <Link to="/login" className="text-rose-500 font-semibold hover:text-rose-600 transition-colors" data-element-id="login-link">Sign in</Link>
        </div>
      </div>
    </div>
  );
